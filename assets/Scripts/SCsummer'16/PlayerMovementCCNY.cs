...
public class PlayerMovementCCNY : MonoBehaviour { 
    ...
    bool justTurned; //Used for new line in log
	...
	void Update()
	{
        if(frameCount == 1)
        {
            trackPosition[0] += frameCount;
            trackPosition[0] += infoBrg.getStartingPosition();
            frameCount++;
        }
        ...
        if (compareTwoVectors(previousPosition, currentPosition) == false)
        {
            trackPosition[0] += frameCount;
            trackPosition[0] += "(" + transform.position.x + ", " + transform.position.y + ", " + transform.position.z + ")" + " \r\n";
            if (justTurned)
                trackPosition[0] += " \r\n";
            frameCount++;
        }
